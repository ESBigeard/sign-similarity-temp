...
def train(args):
    ...
    print("Setting the seed to " + str(args.seed) + " for all random components...")
    ...
    train_set = CzechSLRDataset(args.training_set_path, transform=None, augmentations=False, normalize=False)

    # Validation set
    if args.validation_set == "from-file":
            
        val_set = CzechSLRDataset(
            dataset_filename=args.validation_set_path,
            transform=None,
            augmentations=False,
            normalize=False
        )
    ...
    if args.testing_set_path:
        eval_set = CzechSLRDataset(args.testing_set_path,
                                   transform=None, augmentations=False, normalize=False)
        eval_loader = DataLoader(eval_set, shuffle=True, generator=g)
        print("Testing set size: " + str(len(eval_set)))
    ...
    for epoch in range(args.epochs):
        print("Epoch " + str(epoch + 1) + " of " + str(args.epochs) + " starting...")
        ...
        if val_loader:
            ...
            print("Epoch " + str(epoch + 1) + " of " + str(args.epochs) + " finished with validation accuracy: " + str(val_acc))
